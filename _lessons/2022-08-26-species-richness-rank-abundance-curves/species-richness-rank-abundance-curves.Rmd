...
The Turkey Lakes watershed is located in Ontario, Canada, approximately 50 km North of Sault Ste. Marie. It is 10 km^2^ in area and contains a chain of 4 lakes. The Turkey Lakes Watershed Study (see map) is an initiative by several agencies of the government of Canada, initially designed to study the effects of acid rain on forest ecosystems.
...
data = data.clean # Save for later use in `codyn` tutorial
```
...
Next, we can sum the count data over all the replicate sites, order the species from most abundant to least abundant, and assign a rank, with 1 going to the most abundant species.

```{r}
df.summary1 = df.subset1 %>%
   # Sum counts over replicate sites
  group_by(Species) %>%
  summarise(
    TotalCount = sum(Count))  %>%         
  # Order by species abundance
  arrange(desc(TotalCount)) %>%            
  # Assign a rank to each species
...
Finally, instead of using bars to represent abundances, we can simply use lines to trace the distribution. 
This allows us to compare multiple distributions, in this case, from three different sites with three levels of logging intensity.
